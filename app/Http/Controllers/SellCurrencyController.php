...
class SellCurrencyController extends Controller
{
    ...
    {
        return response()->json(SellCurrency::with(['entry', 'currency', 'account', 'mcCurrency'])->get());
    }
    ...
    public function store(Request $request)
    {
        try{
           ...
           $entry= DB::table('entries')->insertGetId([
            ...
            $document= DB::table('sell_currencies')->insertGetId ([
                'entry_id' => $entry,
            ...
            DB::table('entry_details')->insertGetId([
                'entry_id' => $entry,
                'amount' => -1*$request->mcAmount,
                'currency_id' => $request->mc_currency_id,
                'mcAmount' => -1*$request->mcAmount*$mc_currency->price,
                'account_id' => $request->account_id,
            ...
            $different=($request->mcAmount*$mc_currency->price)-($request->amount*$currency->price);
if($different!=0){
    DB::table('entry_details')->insertGetId([
        'entry_id' => $entry,
...
$response = $entryController->statistics();
            $results=SellCurrency::with(['entry.user', 'currency', 'account', 'mcCurrency'])->where('entry_id',$entry)->first();
            DB::commit();
    ...
    {
        return response()->json(SellCurrency::with(['entry', 'currency', 'account', 'mcCurrency'])->findOrFail($id));
    }
    ...
    public function update(Request $request, $id)
    {
        try{
            DB::beginTransaction();
            $constraint = SellCurrency::findOrFail($id);
            $currency=Currency::where('id',$request->currency_id)->first();
            $mc_currency=Currency::where('id',$request->mc_currency_id)->first();
            DB::table('entries')->where('id',$constraint->entry_id)->update([
                ...
                DB::table('sell_currencies')->where('id',$constraint->id)->update([
                'amount' => $request->amount,
                'price' => $request->price,
                'mcAmount' => $request->mcAmount,
                'currency_id' => $request->currency_id,
...
$entryController = new EntryController();
$response = $entryController->statistics();
            $results=SellCurrency::with(['entry.user', 'currency', 'account', 'mcCurrency'])->where('entry_id',$constraint->entry_id)->first();
            DB::commit();
            return response()->json(['status'=>true,'message'=>'','data'=>$results,'statistics'=>$response], 200);
        }catch(e){
            DB::rollBack();
    ...
    {
        try{
            $constraint = SellCurrency::where('id',$id)->first();
            if(!$constraint){
