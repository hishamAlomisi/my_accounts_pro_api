...
class ConstraintController extends Controller
{
    ...
    {
        return response()->json(Constraint::with(['entry.user', 'currency', 'fromAccount', 'toAccount'])->get());
    }
    ...
    {try{
       ...
       $entry= DB::table('entries')->insertGetId([
        ...
        $document= DB::table('constraints')->insertGetId ([
            'entry_id' => $entry,
        ...
        $results=Constraint::with(['entry.user', 'currency', 'fromAccount', 'toAccount'])->where('entry_id',$entry)->first();
        DB::commit();
    ...
    {
        return response()->json(Constraint::with(['entry.user', 'currency', 'fromAccount', 'toAccount'])->findOrFail($id));
    }
    ...
    {    $currency=Currency::where('id',$request->currency_id)->first();
        ...
        try{
            ...
            DB::table('constraints')->where('id',$constraint->id)->update([
                'amount' => $request->amount,
                'currency_id' => $request->currency_id,
                'from_account_id' =>$request->from_account_id,
                'to_account_id' => $request->to_account_id,
            ...
            $entryController = new EntryController();
            $response = $entryController->statistics();
            $results=Constraint::with(['entry.user', 'currency', 'fromAccount', 'toAccount'])->where('entry_id',$constraint->entry_id)->first();
    ...
    {
        try{
            $constraint = Constraint::where('id',$id)->first();
            if(!$constraint){
