...
class ReceiveController extends Controller
{
    ...
    {
        return response()->json(Receive::with(['entry.user', 'currency', 'fromAccount', 'toAccount'])->get());
    }
    ...
    public function store(Request $request)
    {
        try{
           ...
           $entry= DB::table('entries')->insertGetId([
            ...
            $document= DB::table('receives')->insertGetId ([
                'entry_id' => $entry,
            ...
            $results=Receive::with(['entry.user', 'currency', 'fromAccount', 'toAccount'])->where('entry_id',$entry)->first();
            DB::commit();
    ...
    {
        return response()->json(Receive::with(['entry.user', 'currency', 'fromAccount', 'toAccount'])->findOrFail($id));
    }

    public function update(Request $request, $id)
    {
        $currency=Currency::where('id',$request->currency_id)->first();
        $constraint = Receive::findOrFail($id);
        try{
            $request->validate([
            ...
            DB::table('receives')->where('id',$constraint->id)->update([
                'amount' => $request->amount,
                'receiver' => $request->receiver??"",
                'currency_id' => $request->currency_id,
                'from_account_id' =>$request->from_account_id,
            ...
            $entryController = new EntryController();
            $response = $entryController->statistics();
            $results=Receive::with(['entry.user', 'currency', 'fromAccount', 'toAccount'])->where('entry_id',$constraint->entry_id)->first();
    ...
    {
        try{
            $constraint = Receive::where('id',$id)->first();
            if(!$constraint){
