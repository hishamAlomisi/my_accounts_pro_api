...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('send_transfers', function (Blueprint $table) {
            $table->id();
            $table->foreignId('entry_id')->constrained('entries')->onDelete('cascade'); // معرّف الدخول
            $table->decimal('amount', 15, 2); // المبلغ المحول
            ...
            $table->foreignId('account_id')->constrained('accounts'); // الحساب المحول
            $table->foreignId('agent_account_id')->constrained('accounts'); // حساب الوكيل
            $table->decimal('agentComm', 15, 2); // عمولة الوكيل
            $table->foreignId('agent_comm_currency_id')->constrained('currencies'); // العملة التي يتم حساب عمولة الوكيل بها
    ...
    {
        Schema::dropIfExists('send_transfers');
    }
};
